...
import { proposals, type Proposal } from '@/lib/demo'

const PROPOSALS_KEY = 'kplink-proposals'

type StoredProposal = Proposal

export function ClientProposal({ slug }: { slug: string }) {
  const [customProposals, setCustomProposals] = useState<StoredProposal[]>([])
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        setCustomProposals(parsed)
      }
    } catch {}
  }, [])

  const proposal = useMemo(() => {
    return customProposals.find((item) => item.slug === slug) ?? proposals.find((item) => item.slug === slug) ?? proposals[0]
  }, [customProposals, slug])

  const ctaLink = proposal.faq.find((item) => item.a.startsWith('http'))?.a ?? 'https://t.me/example'
    ...
    <main className="container page-space">
        ...
        <div className="grid">
          {proposal.packages.map((pkg) => (
            <div className={`card ${pkg.featured ? 'featured-card' : ''}`} key={pkg.name}>
              ...
              <a className="btn btn-primary btn-full" href={ctaLink} target="_blank" rel="noreferrer">Написать по этому пакету</a>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="split-section">
        ...
        <div className="card">
          ...
          <Link className="btn btn-primary btn-full" href="/dashboard">Вернуться в дашборд</Link>
        </div>
      </section>
